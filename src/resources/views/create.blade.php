...
@section(config("gmj.backend.content_path"))
    ...
    <div>
        <form id="myform" method="POST"
            action="{{ route('laravel_block_banner_slider_content.store', $element_id) }}" class="relative" enctype="multipart/form-data">
            ...
            <div class="mt-4">
                <label class="block mb-2 cursor-pointer required">
                    Image (1920 x {{$config->height}}) (only accept jpg, png)
                </label>
                <input
                    type="file"
                    name="image"
            ...
            @foreach (config('translatable.locales') as $locale)
                <div class="mt-7">
                    ...
                    <textarea name="content_{{ $locale }}" class="element" style="height: 200px;"></textarea>
                </div>
            @endforeach
